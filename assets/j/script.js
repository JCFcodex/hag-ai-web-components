...
window.addEventListener('load', async () => {
  ...
  const showLoadingScreen = () => {
    ...
    if (!existingLoadingScreen) {
      ...
      loadingScreen.setAttribute('inert', '');
      ...
      loadingScreen.appendChild(waitText);
      circles.forEach((circle) => loadingScreen.appendChild(circle));

      document.body.appendChild(loadingScreen);
    }
  };

  const focusAndPrevent = (event) => {
    if (!event) return;
    const isButton = event.target.closest('button');
    if (!isButton && document.activeElement !== messageInput) {
      messageInput.focus();
    }
  };
      ...
      const processBatch = async (messagesData) => {
        ...
        for (const { type, text, date, image } of messagesData) {
          const messageContent =
            type === 'ai' ? processMarkdown(text) : await text;
  ...
  const applyTooltipHandlers = (element) => {
    let tooltipTimeout;

    const setButtonActive = () => {
      element.style.backgroundColor = '#99b27a';
    ...
    element.addEventListener(eventHandlers.start, setButtonActive);
    element.addEventListener(eventHandlers.end, resetButton);

    if (isMobileDevice()) {
      element.addEventListener(eventHandlers.move, resetButton);
      element.addEventListener(eventHandlers.cancel, resetButton);
    }
  };

  formButtons.forEach(applyTooltipHandlers);
  applyTooltipHandlers(document.getElementById('openModalBtn'));
  applyTooltipHandlers(document.getElementById('changelog-close-modal'));
  ...
  formContainer.addEventListener('click', focusAndPrevent);
...
const categories = {
  ...
  events: [
    ...
    { class: 'fas fa-ticket-alt', color: '#FF5722' },
